...
class Curf:
    ...
    def set_can(self, interface, channel, bitrate, db=None,  app_name= None, test_name=None):
        """ Set the CAN BUS
        Keyword arguments:
        ...
        self.bus = can.interface.Bus(bustype=self.interface,app_name=self.app_name, channel=self.channel, bitrate=self.bitrate)
        self.logbus = can.ThreadSafeBus(interface='virtual', channel=self.channel)
    ...
    def send_frame(self, frame_id, frame_data):
        ...
        # msgDataTx = struct.pack('q',frame_data)
